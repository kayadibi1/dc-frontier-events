...
run surfaces REGRESSIONS -- a source that was healthy and now returns nothing --
so a silently-broken scraper is caught and alerted on, instead of just quietly
shrinking the feed. Health is persisted as JSON (durable across runs) and rendered
to a public status page.
"""
...
def update_health(prior: dict, observations: list[tuple[str, int, str | None]],
    ...
    for slug, count, error in observations:
        ...
        if st == "ok":
            health[slug] = {"slug": slug, "status": "ok", "count": count,
                            "last_success": today, "fail_streak": 0, "reason": ""}
        else:
...
<span class="pill">{n_ok}/{len(rows)} healthy</span></div>
<table><thead><tr><th>Source</th><th>Status</th><th>Events</th>
<th>Last OK</th><th>Detail</th></tr></thead>
<tbody>{''.join(body)}</tbody></table>
<div class="sub" style="margin-top:14px">A source is <b>empty</b> when it fetches but returns
0 events (often a genuinely empty upcoming slate), <b>error</b> when the fetch itself fails.
